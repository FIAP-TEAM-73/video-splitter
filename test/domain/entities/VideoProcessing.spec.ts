...
describe('Create VideoProcessing', () => {
    it('Should create VideoProcessing when it attributes are correct', () => {
        ...
        const interval = 10;
        const sut = new VideoProcessing(email, 'any_zip_link', 'IN_PROGRESS', 'any_bucket_key', interval);
    ...
    it('Should throw an error when creating VideoProcessing with an invalid status', () => {
        const email = new Email('valid_email@mail.com');
        const interval = 10;
        expect(() => new VideoProcessing(email, 'any_zip_link', 'WRONG_STATUS' as any, 'any_bucket_key', interval))
    ...
    it('Should throw an error when creating VideoProcessing with an invalid interval', () => {
        const email = new Email('valid_email@mail.com');
        const interval = 0;
        expect(() => new VideoProcessing(email, 'any_zip_link', 'COMPLETED', 'any_bucket_key', interval))
